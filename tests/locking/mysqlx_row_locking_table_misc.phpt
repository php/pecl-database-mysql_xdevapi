...
<?php
	...
	$res1 = $tab1->select('id', 'n')->where("id like '1'")
		->lockExclusive()->lockShared()->lockShared()->execute();
	check_one($res1, '1', 1);

	$session2->startTransaction();

	$res2 = $tab2->select('id', 'n')->where("id like '2'")
		->lockShared()->lockExclusive()->lockShared()->execute();
	check_one($res2, '2', 2);

	$res2 = $tab2->select('id', 'n')->where("id like '1'")
	...
	check_one($res1, '1', 1);
	$res1 = $tab1->select('id', 'n')->where("id like '3'")
		->lockExclusive()->lockShared()->lockShared()->execute();
	check_one($res1, '3', 3);
	...
	$res2 = $tab2->select('id', 'n')->where("id like '3'")
		->lockExclusive()->lockExclusive()->lockShared()->execute();
	check_one($res2, '3', 3);
