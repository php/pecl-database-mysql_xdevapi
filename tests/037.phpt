...
"hobby": ["PS4","Burattinaio","Fachiro","Calciatore"], "code":6}')->execute();
	...
	expect_eq($res[2]["_id"],4);
        ...
        $res = $coll->find('(1>5) IN (true,false) && {"title":"Spavatore"} NOT IN jobs && _id > 2')->execute()->fetchAll();
...
"hobby": ["Spavare","Dipingere"], "code":2}')->execute();
	...
	expect_eq($res[0]["_id"],1);
        ...
        $res = $coll->find('true IN [(1>5), !(false), (true || false), (false && true)] AND _id > 5')->execute()->fetchAll();
	expect_eq(count($res), 2);
	expect_eq($res[0]["_id"],6);
	expect_eq($res[1]["_id"],7);

        $res = $coll->find('true IN [1-5/2*2 < 3-2/1*2] AND $.code > $._id')->execute()->fetchAll();
	expect_eq(count($res), 2);
	expect_eq($res[0]["_id"],3);
	expect_eq($res[1]["_id"],5);
