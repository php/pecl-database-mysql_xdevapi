...
static void
mysqlx_execute_base_session_query(FILIP_XMYSQLND_NODE_SESSION session,
...
get_schemas_handler_on_row(void * context,
						   FILIP_XMYSQLND_NODE_SESSION const session,
...
mysqlx_base_session_free_storage(zend_object * object)
{
	...
	if (inner_obj) {
		FILIP_XMYSQLND_NODE_SESSION session = inner_obj->session;

		if (session) {
			session->m->free_reference(session.get());
		}
		mnd_efree(inner_obj);
	}
