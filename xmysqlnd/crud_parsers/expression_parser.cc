 ...
 */
Mysqlx::Expr::Expr* Expression_parser::atomic_expr()
{
 ...
 */
void Expression_parser::json_key_value(Mysqlx::Expr::Object* obj)
{
 ...
 */
std::string Expression_parser::cast_data_type_dimension(bool double_dimension)
{
  if (!_tokenizer.cur_token_type_is(Token::LPAREN))
    return "";
  _tokenizer.consume_token(Token::LPAREN);
 ...
 */
std::string Expression_parser::opt_binary()
{
  std::string result;
  const Token& token = _tokenizer.peek_token();
...
{
  return parse_left_assoc_binary_op_expr(_ops.mul_div_expr_types, &Expression_parser::atomic_expr);
}
...
{
  return parse_left_assoc_binary_op_expr(_ops.add_sub_expr_types, &Expression_parser::mul_div_expr);
}
...
{
  return parse_left_assoc_binary_op_expr(_ops.shift_expr_types, &Expression_parser::add_sub_expr);
}
...
{
  return parse_left_assoc_binary_op_expr(_ops.bit_expr_types, &Expression_parser::shift_expr);
}
...
{
  return parse_left_assoc_binary_op_expr(_ops.comp_expr_types, &Expression_parser::bit_expr);
}
...
{
  return parse_left_assoc_binary_op_expr(_ops.and_expr_types, &Expression_parser::ilri_expr);
}
...
{
  return parse_left_assoc_binary_op_expr(_ops.or_expr_types, &Expression_parser::and_expr);
}
 ...
 * expr ::= or_expr
 */
Mysqlx::Expr::Expr* Expression_parser::expr()
{
...
std::string Expression_unparser::any_to_string(const Mysqlx::Datatypes::Any& a)
{
  if (a.type() == Mysqlx::Datatypes::Any::SCALAR)
  {
    return Expression_unparser::scalar_to_string(a.scalar());
  }
  else
  {
...
std::string Expression_unparser::escape_literal(const std::string& s)
{
  std::string result = s;
  Expression_unparser::replace(result, "\"", "\"\"");
  return result;
}

std::string Expression_unparser::scalar_to_string(const Mysqlx::Datatypes::Scalar& s)
{
  switch (s.type())
  {
  ...
  case Mysqlx::Datatypes::Scalar::V_UINT:
    ...
    case Mysqlx::Datatypes::Scalar::V_OCTETS:
    {
      ...
      return "\"" + Expression_unparser::escape_literal(value) + "\"";
    }
...
std::string Expression_unparser::document_path_to_string(const ::google::protobuf::RepeatedPtrField< ::Mysqlx::Expr::DocumentPathItem >& dp)
{
  std::string s;
  std::vector<std::string> parts;
...
std::string Expression_unparser::column_identifier_to_string(const Mysqlx::Expr::ColumnIdentifier& colid)
{
  std::string s = Expression_unparser::quote_identifier(colid.name());
  if (colid.has_table_name())
  {
    s = Expression_unparser::quote_identifier(colid.table_name()) + "." + s;
  }
  if (colid.has_schema_name())
  {
    s = Expression_unparser::quote_identifier(colid.schema_name()) + "." + s;
  }
  std::string dp = Expression_unparser::document_path_to_string(colid.document_path());
  if (!dp.empty())
    s = s + "$" + dp;
  return s;
}

std::string Expression_unparser::function_call_to_string(const Mysqlx::Expr::FunctionCall& fc)
{
  std::string s = Expression_unparser::quote_identifier(fc.name().name()) + "(";
...
std::string Expression_unparser::operator_to_string(const Mysqlx::Expr::Operator& op)
{
  ...
  {
    std::string result = "INTERVAL " + Expression_unparser::expr_to_string(ps.Get(0)) + " ";
    std::string data = Expression_unparser::expr_to_string(ps.Get(1));
    Expression_unparser::replace(data, "\"", "");
    return result + data;
  }
  else if (name == "BETWEEN")
  {
    std::string result = Expression_unparser::expr_to_string(ps.Get(0));
    result += " BETWEEN ";
    result += Expression_unparser::expr_to_string(ps.Get(1));
    result += " AND ";
    result += Expression_unparser::expr_to_string(ps.Get(2));
    return result;
  }
  else if (name == "LIKE" && ps.size() == 3)
  {
    return Expression_unparser::expr_to_string(ps.Get(0)) + " LIKE " + Expression_unparser::expr_to_string(ps.Get(1)) + " ESCAPE " + Expression_unparser::expr_to_string(ps.Get(2));
  }
  else if (ps.size() == 2)
  {
    std::string result = "(" + Expression_unparser::expr_to_string(ps.Get(0)) + " " + op.name() + " " + Expression_unparser::expr_to_string(ps.Get(1)) + ")";
    return result;
  }
  else if (ps.size() == 1)
  {
    ...
    if (name_.size() == 1)
    {
      return name_ + Expression_unparser::expr_to_string(ps.Get(0));
    }
    else
    {
      // something like NOT
...
void Expression_unparser::replace(std::string& target, const std::string& old_val, const std::string& new_val)
{
...
std::string Expression_unparser::quote_identifier(const std::string& id)
{
...
std::string Expression_unparser::expr_to_string(const Mysqlx::Expr::Expr& e)
{
  if (e.type() == Mysqlx::Expr::Expr::LITERAL)
  {
    return Expression_unparser::scalar_to_string(e.literal());
  }
  else if (e.type() == Mysqlx::Expr::Expr::IDENT)
  {
    return Expression_unparser::column_identifier_to_string(e.identifier());
  }
  else if (e.type() == Mysqlx::Expr::Expr::FUNC_CALL)
  {
    return Expression_unparser::function_call_to_string(e.function_call());
  }
  else if (e.type() == Mysqlx::Expr::Expr::OPERATOR)
  {
    return Expression_unparser::operator_to_string(e.operator_());
  }
  else if (e.type() == Mysqlx::Expr::Expr::VARIABLE)
  {
    return std::string("$") + Expression_unparser::quote_identifier(e.variable());
  }
  else if (e.type() == Mysqlx::Expr::Expr::OBJECT)
  {
    return Expression_unparser::object_to_string(e.object());
  }
  else if (e.type() == Mysqlx::Expr::Expr::PLACEHOLDER)
  {
    return Expression_unparser::placeholder_to_string(e);
  }
  else if (e.type() == Mysqlx::Expr::Expr::ARRAY)
  {
    return Expression_unparser::array_to_string(e);
  }
  else
  {
...
std::string Expression_unparser::placeholder_to_string(const Mysqlx::Expr::Expr& e)
{
  std::string result = ":" + boost::lexical_cast<std::string>(e.position());
  return result;
}

std::string Expression_unparser::array_to_string(const Mysqlx::Expr::Expr& e)
{
  std::string result = "[ ";
  ...
  for (int i = 0; i < a.value_size(); i++)
  {
    ...
    result += Expression_unparser::expr_to_string(a.value(i));
  }
...
std::string Expression_unparser::object_to_string(const Mysqlx::Expr::Object& o)
{
  ...
  for (int i = 0; i < o.fld_size(); ++i)
  {
    ...
    result += "'" + fld.key() + "' : ";
    result += Expression_unparser::expr_to_string(fld.value());
  }
  result += " }";
  return result;
}

std::string Expression_unparser::column_to_string(const Mysqlx::Crud::Projection& c)
{
  std::string result = Expression_unparser::expr_to_string(c.source());
...
std::string Expression_unparser::order_to_string(const Mysqlx::Crud::Order& c)
{
  std::string result = Expression_unparser::expr_to_string(c.expr());
